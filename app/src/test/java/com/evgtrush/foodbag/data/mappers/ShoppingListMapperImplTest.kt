...
class ShoppingListMapperImplTest {

    private lateinit var mapper: ShoppingListMapper
    private lateinit var shoppingListEntity: ShoppingListEntity
    private lateinit var shoppingList: ShoppingList

    @Before
    fun setUp() {
        mapper = ShoppingListMapperImpl()
        shoppingListEntity =
            ShoppingListEntity(
                uid = 1,
                name = "Покупочки"
    ...
    fun `convert() - data model is converted correctly to domain model`() {
        runBlocking {
            val result = mapper.convert(shoppingListEntity)

            Truth.assertThat(result).isEqualTo(shoppingList)
        }
    }
}