...
internal class ToDoKaListsAdapter(
    ...
    private val viewModel: ToDoKaListsViewModel
) : RecyclerView.Adapter<ToDoKaListsAdapter.ToDoKaListViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ToDoKaListViewHolder, position: Int) =
        holder.bindView(toDoKaLists[position], fragmentManager, viewModel)
    ...
    internal class ToDoKaListViewHolder(private val binding: ListItemToDoKaListBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bindView(toDoKaList: ToDoKaList, fragmentManager: FragmentManager, viewModel: ToDoKaListsViewModel) {
            with(binding) {
                title.text = toDoKaList.name
                progress.progress = toDoKaList.progress
                ready.visibility = if (toDoKaList.progress >= 100) {
                    View.VISIBLE
                } else {
                    View.GONE
                }
                ...
                more.setOnClickListener {
                    val modalBottomSheet = EditToDoKaListBottomSheet(toDoKaList, viewModel)
                    modalBottomSheet.show(fragmentManager, EditToDoKaListBottomSheet.TAG)
                }
            }
        }
    }
